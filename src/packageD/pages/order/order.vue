        ...
        <scroll-view class="list-scroll-content" scroll-y @scrolltolower="loadData">
          ...
          <view v-for="(order, index) in tabItem.orderList" :key="index" class="order-item">
            ...
            <view class="i-top b-b">
              ...
              <!--              给不同订单状态设置不同的颜色-->
              <!-- 否则显示订单状态 -->
              <text v-else class="status" :style="{ color: getStatusColor(order.status) }">
            ...
            <view class="order-content" @click="goToOrderDetail(order)">
              ...
              >
                ...
                >
                  ...
                  <view class="right">
                    ...
                    <text class="attr-box">
                      {{ orderItem.skuName || "" }} x {{ orderItem.quantity || 1 }}
                    </text>
              ...
              <view class="price-box">
                共
                <text class="num">{{ order.totalQuantity }}</text>
                件商品 实付款
            ...
            <view class="action-box b-t" v-if="order.status === 1">
              <!-- 新增：查看详情按钮 -->
              <button class="action-btn secondary" @click="goToOrderDetail(order)">查看详情</button>
              <button class="action-btn" @click="cancelOrder(order)">取消订单</button>
              <button class="action-btn recom" @click="doPay(order)">立即支付</button>
            </view>

            <!-- ✅ 新增：待发货状态显示申请退款按钮 -->
            <view class="action-box b-t" v-else-if="order.status === 2">
              <!-- 新增：查看详情按钮 -->
              <button class="action-btn secondary" @click="goToOrderDetail(order)">查看详情</button>
              <button class="action-btn" @click="applyRefund(order)">申请退款</button>
            </view>

            <!-- ✅ 新增：已发货状态显示申请退款/退货按钮 -->
            <view class="action-box b-t" v-else-if="order.status === 3">
              <!-- 新增：查看详情按钮 -->
              <button class="action-btn secondary" @click="goToOrderDetail(order)">查看详情</button>
              <button class="action-btn" @click="applyRefund(order, 'refund')">申请退款</button>
              <button class="action-btn recom" @click="applyRefund(order, 'return')">
                申请退货
              </button>
            </view>

            <!-- ✅ 新增：已完成状态显示申请售后按钮 -->
            <view class="action-box b-t" v-else-if="order.status === 4 && canApplyAfterSale(order)">
              <!-- 新增：查看详情按钮 -->
              <button class="action-btn secondary" @click="goToOrderDetail(order)">查看详情</button>
              <button class="action-btn" @click="applyAfterSale(order)">申请售后</button>
            </view>

            <!-- ✅ 新增：已关闭/已取消状态 -->
            <view class="action-box b-t" v-else-if="order.status === 5 || order.status === 6">
              <!-- 新增：查看详情按钮 -->
              <button class="action-btn secondary" @click="goToOrderDetail(order)">查看详情</button>
            ...
            <!-- ✅ 新增：其他状态（如已完成但不可申请售后） -->
            <view class="action-box b-t" v-else-if="order.status === 4">
              <!-- 新增：查看详情按钮 -->
              <button class="action-btn secondary" @click="goToOrderDetail(order)">查看详情</button>
            </view>
          </view>
